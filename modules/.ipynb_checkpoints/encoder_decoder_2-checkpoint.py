...
class Transformer(nn.Module):
    ...
    def encode(self, src, src_mask, tags=None, fc_feats=None):
        ...
        if tags is not None:
            ...
            # print(attened_visual_feats.shape, src_mask.shape)
    ...
    def decode(self, hidden_states, src_mask, tgt, tgt_mask, tags=None, mode=None):
        ...
        if mode is not None:
            ...
            #训练阶段，交叉注意力融合视觉和标签特征;验证测试阶段，融合视觉特征与预测的标签特征
            ...
            #print(tgt_embedding.shape, tgt_mask.shape)

            #tgt_cls_embedding = torch.cat((tgt_embedding, tgt_embedding_cls.unsqueeze(dim=1)), dim=1)
              # print(tgt_cls_embedding.shape)
            ...
            # # 简单的映射，从视觉特征到标签特征
            # if self.linear_val_test is None:
            #     self.linear_val_test = nn.Linear(hidden_states.shape[1], 365).to(hidden_states.device)
            tag_pred_embedding = self.linear_train(hidden_states.permute(0, 2, 1)).permute(0, 2, 1)
            # 验证测试阶段，融合视觉特征与预测的标签特征
            hidden_states = self.sublayer(hidden_states,
                                          lambda hidden_state: attention_cls(hidden_states, tag_pred_embedding,
                                                                             tag_pred_embedding))

            #tgt_cls_embedding = torch.mean(tag_pred_embedding, dim=1).unsqueeze(dim=1)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, hidden_states, src_mask, tgt_mask, memory, tgt_cls_embedding=None):
        #print(hidden_states.shape, src_mask.shape, x.shape, tgt_mask.shape, tgt_cls_embedding.shape)
        m = hidden_states
        ...
        if tgt_cls_embedding is not None:
            ...
            #print(src_mask.shape, m_cls.shape)
            # extra_column = torch.ones(src_mask.size(0), src_mask.size(1), 2, dtype=torch.bool).to(src_mask.device)
            ...
            for i in range(m_cls.shape[1]-hidden_states.shape[1]):
                ...
                # 沿着第三个维度拼接
                src_mask = torch.cat([src_mask, extra_column], dim=2)
            # extra_row = torch.ones(tgt_mask.size(0), 1, tgt_mask.size(2), dtype=torch.bool).to(tgt_mask.device)
            # tgt_mask = torch.cat([tgt_mask, extra_row], dim=1)
...
class EncoderDecoder(AttModel):
    ...
    def __init__(self, args, tokenizer):
        ...
        self.fc_linear = nn.Linear(self.args.d_vf*2, self.args.d_model)
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks, tags):#验证和测试阶段使用的是预测的tag
        #fc_feats = self.ssl_model(self.fc_linear(fc_feats), tags).unsqueeze(dim=1)
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks)
        # print(att_feats.shape, att_masks.shape)

        memory, att_masks = self.model.encode(att_feats, att_masks, tags=tags, fc_feats=fc_feats)
        # memory = torch.cat((memory, fc_feats), dim=1)
        # att_masks = memory.new_ones(memory.shape[:2], dtype=torch.long)
        # att_masks = att_masks.unsqueeze(-2)
        #print(memory.shape, att_masks.shape)
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None, tags=None):
        ...
        out, tag_embedding, tag_pred_embedding = self.model(fc_feats, att_feats, seq, att_masks, seq_mask, tags)#传入真实的tag
        # print(out.shape)
